...
public class TrapSpawner : MonoBehaviourPun
{
    ...
    public class Trap
    {
        ...
        public GameObject trapInstance; // Made public
    ...
    [SerializeField] public Trap poisonDartTrap; // Made public
    [SerializeField] public Trap bouncingBettyTrap; // Made public
    [SerializeField] public Trap detonatorTrap; // Made public
    [SerializeField] public Trap ceilingTrap; // Made public
    ...
    public Trap currentTrap; // Made public
            ...
            foreach (var entry in trapKeyMap)
            {
                ...
                if (Input.GetKeyDown(entry.Key))
                {
                    ...
                    if (HUDManager.Instance.GetCD(trapIndex)) {
                        // todo Add HUD notifier that trap is on CD    
                        return;
                    }
                ...
                if (Input.GetMouseButtonDown(0) && isPlacingTrap && canPlaceTrap && !HUDManager.Instance.GetCD(trapIndex))
                {
                    if(currentTrap == detonatorTrap)
                    {
                        ...
                        io.SetTrap(io, currentTrap.trapInstance, PhotonNetwork.LocalPlayer.ActorNumber);
                        // StartCoroutine(AwaitTrapInstance(io));
    ...
    public IEnumerator AwaitTrapInstance(InteractableObject io)
    {
        if (currentTrap.trapInstance == null) {
            yield return new WaitForSeconds(.1f); 
            Debug.Log("trap instance is null");
        }
        while (io.trapInstance is null) {
            Debug.LogWarning("Attempting to retrieve trap instance");
            currentTrap.trapInstance.transform.SetParent(io.transform);
            Debug.LogWarning($"currentTrap.trapInstance inside Coroutine = {currentTrap.trapInstance}");
        ...
        Debug.Log($"Found trapinstance and assigned {currentTrap.trapInstance.name}");
    }
