...
public class PlayerStatsManager : MonoBehaviourPunCallbacks
{
    ...
    public readonly string[] propertyKeys = { "win",
            "keycardsCollected", "trapsSet", "trap1", "trap2", "trap3", "trap4",
            "kills", "trap1kill", "trap2kill", "trap3kill", "trap4kill",
            "deaths", "killedbyp1", "killedbyp2", "killedbyp3", "killedbyp4", "player1kill", "player2kill", "player3kill", "player4kill"
        };
    private string[] playerNames = { "Elise", "Hazel", "Sofia", "Amelia" };
    ...
    public const string Player1Kill= "player1kill";
    public const string Player2Kill= "player2kill";
    public const string Player3Kill= "player3kill";
    public const string Player4Kill= "player4kill";
    ...
    public void UpdateKills(int trapType, int ownerID, int[] victims)
    {
        ...
        #endregion
        // update the specific trap type kills
        int currentTrapKillVal = GetCustomProperty($"trap{trapType}kill");
        int updatedTrapKillVal = currentTrapKillVal + victims.Length;
        string propTrapKillToUpdate = $"trap{trapType}kill";
        SetCustomProperty(propTrapKillToUpdate, updatedTrapKillVal); 
        //update the player killed 
        int currentPlayerKillVal = GetCustomProperty($"player{trapType}kill");
        int updatedPlayerKillVal = currentTrapKillVal + victims.Length;
        string propPlayerKillToUpdate = $"player{victims[0]}kill"; 
        SetCustomProperty(propTrapKillToUpdate, updatedTrapKillVal); 
        //update total kills
